...
def get_flash_history():
    try:
        ...
        flashed_df = df[
            df["Flashed"] & pd.to_datetime(df["date_flash"], errors="coerce").notna()
        ].copy()
...
def sync_with_uid():
    ...
    try:
        ...
        for index, row in df.iterrows():
            ...
            if isinstance(invader_id, str) and "_" in invader_id:
                ...
                if suffix.isdigit():
                    ...
                    if normalized_id in api_invader_ids:
                        ...
                        df.at[index, "date_flash"] = flashed_invaders_from_api[
                            normalized_id
                        ].get("date_flash")
                        df.at[index, "image_url"] = flashed_invaders_from_api[
                            normalized_id
                        ].get("image_url")
                        continue
            ...
            if invader_id in api_invader_ids:
                if not row["Flashed"]:
                    updated_count += 1
                df.at[index, "Flashed"] = True
                df.at[index, "date_flash"] = flashed_invaders_from_api[invader_id].get(
                    "date_flash"
                )
                df.at[index, "image_url"] = flashed_invaders_from_api[invader_id].get(
                    "image_url"
                )
            else:
        ...
        return jsonify(
            {"status": "error", "message": f"Failed to call external API: {e}"}
        ), 502
    except Exception as e:
...
def process_synced_data():
    try:
        ...
        if not api_data:
            return jsonify(
                {"status": "error", "message": "No invadersData provided"}
            ), 400
...
def upload_video():
    ...
    if video_file and allowed_file(video_file.filename):
        ...
        try:
            ...
            new_flashed = [
                inv_id
                for inv_id in invader_ids
                if inv_id in df["id"].values
                and not df[df["id"] == inv_id]["Flashed"].iloc[0]
            ]
...
def update_from_pnote():
    ...
    try:
        ...
        if not pnote_invaders:
            return jsonify(
                {"status": "error", "message": "No invaders data provided"}
            ), 400
        ...
        for index, row in df.iterrows():
            ...
            if invader_id in pnote_dict:
                ...
                current_status = pnote_inv.get(
                    "status", ""
                ).lower()
                current_flashable = row["Flashable"]
                current_comment = str(row.get("comment", ""))

                if current_status in ["ok", "damaged"]:
                    if not current_flashable:
                        df.at[index, "Flashable"] = True
                        status_changed_count += 1

                    if (
                        current_status == "damaged"
                        and "damaged" not in current_comment.lower()
                    ):
                        if current_comment:
                            new_comment = current_comment + " | damaged"
                        else:
                            new_comment = "damaged"
                        df.at[index, "comment"] = new_comment

                elif current_status != "" and current_flashable:
                    df.at[index, "Flashable"] = False
                    new_comment = (
                        current_comment
                        + f" | Non flashable car status = {current_status} d'après les dernières données"
                        if current_comment
                        else f"Non flashable car status = {current_status} d'après les dernières données"
                    )
                    df.at[index, "comment"] = new_comment
                    status_changed_count += 1
        ...
        if missing_in_pnote:
            ...
            print(
                f"Invaders dans le CSV mais pas dans pnote.eu: {invaders_not_in_pnote}"
            )
...
def get_invaders_by_city(city_code):
    ...
    try:
        ...
        return jsonify({"invaders": formatted_data, "city_info": city_info})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
